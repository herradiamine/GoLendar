...
func setupTestRouter() *gin.Engine {
	...
	router.GET("/user-calendar/:id", func(c *gin.Context) { UserCalendar.Get(c) })
	router.POST("/user-calendar", func(c *gin.Context) { UserCalendar.Add(c) })
	router.PUT("/user-calendar/:id", func(c *gin.Context) { UserCalendar.Update(c) })
	router.DELETE("/user-calendar/:id", func(c *gin.Context) { UserCalendar.Delete(c) })
...
func TestUserCalendarCRUD(t *testing.T) {
	...
	t.Run("Create User Calendar Link", func(t *testing.T) {
		payload := map[string]interface{}{
			"user_id":     userID2, // Utiliser le deuxième utilisateur
	...
	t.Run("Get User Calendar Link", func(t *testing.T) {
		req, _ := http.NewRequest("GET", fmt.Sprintf("/user-calendar/%d", userCalendarID), nil)
		w := httptest.NewRecorder()
		router.ServeHTTP(w, req)
	...
	t.Run("Update User Calendar Link", func(t *testing.T) {
		// Créer un troisième utilisateur pour le test de mise à jour
		uniqueEmail3 := fmt.Sprintf("usercalendar.user3+%d@test.com", time.Now().UnixNano())
		var userID3 int
		{
			payload := common.CreateUserRequest{
				Lastname:  "Test3",
				Firstname: "UserCalendar3",
				Email:     uniqueEmail3,
				Password:  "motdepasse123",
			}
			jsonData, _ := json.Marshal(payload)
		...
		jsonData, _ := json.Marshal(payload)
		req, _ := http.NewRequest("PUT", fmt.Sprintf("/user-calendar/%d", userCalendarID), bytes.NewBuffer(jsonData))
		req.Header.Set("Content-Type", "application/json")
	...
	t.Run("Delete User Calendar Link", func(t *testing.T) {
		req, _ := http.NewRequest("DELETE", fmt.Sprintf("/user-calendar/%d", userCalendarID), nil)
		w := httptest.NewRecorder()
		router.ServeHTTP(w, req)
...
func TestUserCalendarErrorCases(t *testing.T) {
	...
	t.Run("Get Non-existent User Calendar Link", func(t *testing.T) {
		req, _ := http.NewRequest("GET", "/user-calendar/999", nil)
		w := httptest.NewRecorder()
		router.ServeHTTP(w, req)
