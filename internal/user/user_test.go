...
// TestGetUserByIdRoute teste la route GET /user/:user_id (admin) avec plusieurs cas
func TestGetUserByIdRoute(t *testing.T) {
	...
	var TestCases = []struct {
		...
		{
			CaseName: "Récupération réussie d'un utilisateur par ID (admin)",
			CaseUrl:  "/user/1",
			SetupData: func() map[string]interface{} {
				// Créer un utilisateur admin et un utilisateur cible
				adminUser, err := testutils.GenerateAuthenticatedUser(true, true)
				require.NoError(t, err)

				targetUser, err := testutils.GenerateTestUser()
				require.NoError(t, err)

				// Retourner les données de préparation
				return map[string]interface{}{
					"admin":  adminUser,
					"target": targetUser,
					"_headers": map[string]string{
			...
			ExpectedHttpCode: http.StatusOK,
			ExpectedMessage:  common.MsgSuccessGetUser,
			ExpectedError:    "",
		},
		{
			...
			SetupData: func() map[string]interface{} {
				...
				// Retourner les données de préparation sans header d'authentification
				return map[string]interface{}{
					"target": targetUser,
					"_headers": map[string]string{
						"Content-Type": "application/json",
		...
		{
			CaseName: "Échec de récupération sans privilèges admin",
			CaseUrl:  "/user/1",
			SetupData: func() map[string]interface{} {
				// Créer un utilisateur normal (non-admin) et un utilisateur cible
				normalUser, err := testutils.GenerateAuthenticatedUser(false, true)
				require.NoError(t, err)
	...
	for _, testCase := range TestCases {
		t.Run(testCase.CaseName, func(t *testing.T) {
			...
			if testCase.ExpectedHttpCode == http.StatusOK {
				...
				// Vérifier que les données utilisateur sont présentes dans la réponse
				require.NotNil(t, response.Data, "Les données utilisateur devraient être présentes")
			} else {
				require.False(t, response.Success, "La réponse devrait être un échec")
				require.Equal(t, testCase.ExpectedError, response.Error, "Message d'erreur incorrect")
...
// TestUpdateUserByIdRoute teste la route PUT /user/:user_id (admin) avec plusieurs cas
func TestUpdateUserByIdRoute(t *testing.T) {
	...
	var TestCases = []struct {
		...
		{
			CaseName: "Modification réussie d'un utilisateur par ID (admin)",
			CaseUrl:  "/user/1",
			RequestData: func() map[string]interface{} {
				return map[string]interface{}{
					"lastname":  "Martin",
		...
		{
			CaseName: "Échec de modification sans authentification",
			CaseUrl:  "/user/1",
			RequestData: func() map[string]interface{} {
				return map[string]interface{}{
					"lastname": "Martin",
		...
		{
			CaseName: "Échec de modification sans privilèges admin",
			CaseUrl:  "/user/1",
			RequestData: func() map[string]interface{} {
				return map[string]interface{}{
					"lastname": "Martin",
		...
		{
			CaseName: "Échec de modification avec utilisateur inexistant",
			CaseUrl:  "/user/99999",
			RequestData: func() map[string]interface{} {
				return map[string]interface{}{
					"lastname": "Martin",
	...
	for _, testCase := range TestCases {
		t.Run(testCase.CaseName, func(t *testing.T) {
			...
			// Convertir les données de requête en JSON
			jsonData, err := json.Marshal(requestData)
			require.NoError(t, err, "Erreur lors de la sérialisation des données de requête")
			...
			if testCase.ExpectedHttpCode == http.StatusOK {
				...
				// Vérifier que les modifications ont été appliquées en base
				if target, ok := setupData["target"].(*testutils.TestUser); ok {
					...
					err := common.DB.QueryRow(`
						...
						FROM user 
						WHERE user_id = ?
					`, target.UserID).Scan(&updatedUser.Lastname, &updatedUser.Firstname, &updatedUser.Email)
					require.NoError(t, err, "Erreur lors de la vérification des modifications")

					// Vérifier que les nouvelles valeurs correspondent
					if newLastname, ok := requestData["lastname"].(string); ok {
						require.Equal(t, newLastname, updatedUser.Lastname, "Le nom de famille n'a pas été mis à jour")
					}
					if newFirstname, ok := requestData["firstname"].(string); ok {
						require.Equal(t, newFirstname, updatedUser.Firstname, "Le prénom n'a pas été mis à jour")
					}
					if newEmail, ok := requestData["email"].(string); ok {
						require.Equal(t, newEmail, updatedUser.Email, "L'email n'a pas été mis à jour")
					}
				}
			} else {
				require.False(t, response.Success, "La réponse devrait être un échec")
...
// TestDeleteUserByIdRoute teste la route DELETE /user/:user_id (admin) avec plusieurs cas
func TestDeleteUserByIdRoute(t *testing.T) {
	...
	var TestCases = []struct {
		...
		{
			CaseName: "Suppression réussie d'un utilisateur par ID (admin)",
			CaseUrl:  "/user/1",
			SetupData: func() map[string]interface{} {
				// Créer un utilisateur admin et un utilisateur cible
				adminUser, err := testutils.GenerateAuthenticatedUser(true, true)
			...
			ExpectedHttpCode: http.StatusOK,
			ExpectedMessage:  common.MsgSuccessDeleteUser,
			ExpectedError:    "",
		},
		{
		...
		{
			CaseName: "Échec de suppression sans privilèges admin",
			CaseUrl:  "/user/1",
			SetupData: func() map[string]interface{} {
				// DOIT CONTENIR L'ENSEMBLE DES INSTRUCTIONS QUI PREPARENT LE CAS A LA RECEPTION DE LA REQUEST GET/DELETE
				// Créer un utilisateur normal (non-admin) et un utilisateur cible
...
// TestGetUserWithRolesRoute teste la route GET /user/:user_id/with-roles (admin) avec plusieurs cas
func TestGetUserWithRolesRoute(t *testing.T) {
	// TestCases contient les cas qui seront testés
	var TestCases = []struct {
		CaseName         string
		CaseUrl          string
		...
		{
			CaseName: "Récupération réussie d'un utilisateur avec ses rôles (admin)",
			CaseUrl:  "/user/1/with-roles",
			SetupData: func() map[string]interface{} {
				// Créer un utilisateur admin et un utilisateur cible avec rôles
				adminUser, err := testutils.GenerateAuthenticatedUser(true, true)
				require.NoError(t, err)
				...
				// Assigner des rôles à l'utilisateur cible
				_, err = common.DB.Exec(`
	...
	for _, testCase := range TestCases {
		t.Run(testCase.CaseName, func(t *testing.T) {
			...
			if testCase.ExpectedHttpCode == http.StatusOK {
				...
				// Vérifier que les données utilisateur avec rôles sont présentes dans la réponse
				require.NotNil(t, response.Data, "Les données utilisateur avec rôles devraient être présentes")
			} else {
				require.False(t, response.Success, "La réponse devrait être un échec")
				require.Equal(t, testCase.ExpectedError, response.Error, "Message d'erreur incorrect")
				require.Empty(t, response.Message, "Pas de message de succès attendu")
...
// TestGetAuthMeRoute teste la route GET /auth/me avec plusieurs cas
func TestGetAuthMeRoute(t *testing.T) {
	...
	var TestCases = []struct {
		...
		{
			CaseName: "Récupération réussie des données de l'utilisateur connecté via auth",
			CaseUrl:  "/auth/me",
			SetupData: func() map[string]interface{} {
